...
/**
 * Searches document titles and OCR page text. Results are grouped to the best hit per document,
 * title matches first, then page-text matches ranked by bm25.
 */
export async function searchDocuments(db: SqlDb, input: string, limit = 50): Promise<SearchHit[]> {
  ...
  const rows = await db.getAllAsync<HitRow>(
    `WITH title_hits AS (
       SELECT d.id AS document_id, d.title, NULL AS page_number, NULL AS snippet,
              0 AS kind, bm25(title_fts) AS rank
       FROM title_fts JOIN documents d ON d.rowid = title_fts.rowid
       WHERE title_fts MATCH ?1 AND d.deleted_at IS NULL
     ),
     text_hits AS (
       SELECT d.id AS document_id, d.title, p.position + 1 AS page_number,
              snippet(ocr_fts, 0, '[', ']', '…', 12) AS snippet,
              1 AS kind, bm25(ocr_fts) AS rank
       FROM ocr_fts
     ...
     ranked AS (
       SELECT *, ROW_NUMBER() OVER (PARTITION BY document_id ORDER BY kind, rank) AS rn
       FROM (SELECT * FROM title_hits UNION ALL SELECT * FROM text_hits)
     )
     SELECT document_id, title, page_number, snippet FROM ranked
     WHERE rn = 1
     ORDER BY kind, rank
     LIMIT ?2`,
    query,
